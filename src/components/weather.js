...
class Weather extends React.Component{
	render(){
		let color = 'lightblue';
		let icon = [];
		// light rain, clear sky, scattered clouds, moderate rain, broken clouds, few clouds, overcast clouds, few clouds
		/*      'CLEAR_DAY',
      'CLEAR_NIGHT',
      'PARTLY_CLOUDY_DAY',
      'PARTLY_CLOUDY_NIGHT',
      ...
      'FOG' */
		...
		if(this.props.description){
			for(var i=0; i < 5; i++){
				...
				if(string === "broken clouds" || string === "few clouds" || string === "scattered clouds"  || string === "few clouds"){
