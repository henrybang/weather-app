...
class App extends React.Component {
  // TODO: add high and low temps, restyle 
  state = {
    unit: "kelvin",
    date: undefined,
  ...
  // geolocation before hitting submit
  setPosition = async () => {
    getLocation()
    // location forecast
    const api_call = await fetch(`http://api.openweathermap.org/data/2.5/forecast?lat=${latitude}&lon=${longitude}&APPID=${API_KEY}`);
    // api return parse to json format
    const data = await api_call.json();

    console.log(data); 
    this.getWeatherGeolocation(data);
  }

  getWeatherGeolocation = (data) => {
